...
import { buildSchema, renderTreeMd, upsertByPath, importOutline, applyBatch, getCommitDiff, getNodeDiffs, getCommitTrack, getNodeTracks, getCombinedDiff, getNodeDuplicates, runTestCases, renderAcceptanceMd, renderAcceptanceStatusMd, renderTestReportMd, runReleaseChecks, renderReleaseChecklistMd, renderReleaseSqlAuditMd, renderBusinessGateMd, renderReadinessMd, renderMindmapMd, renderCodeAuditMd, getNodeCodeAudit, renderSecretScanMd, renderDeliveryGateMd, renderDeliverySnapshotMd, renderDesignOutlineMd, applyDesignOutline } from './ops.mjs'
...
export function createMcpServer({ store }) {
  ...
  server.tool(
    'business_gate',
    '业务检查门禁：判定节点（含可选子树）的未关闭缺陷是否清干净、启用中的 biz_check 用例最近结论是否全部 pass；format=md 返回可贴进 issue 的 markdown',
    { node: z.union([z.number(), z.string()]), scope: z.string().optional(), format: z.string().optional() },
    mcpValidate(async ({ node, scope, format }) => {
      const n = store.resolveRef(String(node))
      const gate = store.buildBusinessGate(n.id, { scope })
      const text = store.normalizeFormat(format) === 'md' ? renderBusinessGateMd(gate) : JSON.stringify(gate, null, 2)
      return { content: [{ type: 'text', text }] }
    })
  )
