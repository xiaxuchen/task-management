...
- `server/git.mjs`：`branchContains` / `commitTrack`（`merge-base --is-ancestor`，经 `gitTry` 以退出码表达结果）
...
- 前端 tracks 异步加载（不阻塞抽屉主信息），`repo@sha` 映射后驱动徽标
- 徽标三色：绿（已合入）/ 灰（未合入）/ 浅灰（未配置或无 ref），悬停显示分支名与原因
