...
| 39 | 上线 SQL 风险审查由既有 `release_items` 推导，不落表 | 上线 SQL 以 `kind=sql` 的 release item 登记（名称 + 内容 + 回滚 + 状态）。「内容有没有风险」是从这份内容推导出的**结论**，不是新的业务数据；落表会产生第二份真相，还要处理「内容改了结论要不要失效」。与 `acceptance_report` / `readiness` / `release_checklist` / 推送门禁同一条纯读原则：`buildReleaseSqlAudit` **不写库、不动 revision**，AI 可高频轮询而不制造变更噪声 |
| 40 | 危险 SQL 与风险提示分级：`danger` 阻塞、`warn` 只提示 | 把「不可逆破坏」与「风险写法 / 缺回滚」混为一谈会失去可执行性——前者必须拦住上线，后者只该提示补全。因此 `DROP TABLE/DATABASE` / `TRUNCATE` / 无 `WHERE` 的 `UPDATE` / `DELETE` 记 `danger` → `ready=false` 且进 `blockers`；`DROP COLUMN` / 缺回滚脚本记 `warn` → 单列 `warnings`，不改变 `ready`。无 `WHERE` 判定必须在**单条语句内**（按 `;` 切分）进行，否则同段带 `WHERE` 的语句会把无条件的 `UPDATE` / `DELETE` 洗白；匹配前剥离注释避免关键字误伤。决策编号依赖 `../commit-push-gate/` 分支先合入（36/37/38），合并顺序决定最终编号 |
