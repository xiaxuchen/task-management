...
| 39 | 文档敏感信息扫描纯读且证据必须脱敏 | AI 与人在需求 / 设计 / 联调文档里频繁粘贴 token、curl 示例与配置片段，这些文档会继续进入 issue / MR / agent 上下文。扫描是从既有 `documents` 推导出的安全结论，不落表、不 bump revision（与 readiness / acceptance / delivery-gate 同原则）；有高危命中 `ready=false`，只有提示或未命中 `ready=true`，没有非空文档 `ready=null`（空态不是安全通过）。扫描报告本身就是新的传播面，所有命中证据必须先脱敏：JSON / MCP / markdown 都只返回掩码值与 `[REDACTED:<rule>]` 上下文，绝不回显原始凭据。首版不自动并入交付门禁——那会改变既有 `totals.sources` 契约，属下一步显式决策 |
| 40 | 同一行多凭据的 excerpt 必须整行统一脱敏 | 独立验收 D1：同一行有多个凭据时，逐条 finding 只替换自己的命中片段，会把其他凭据原文留在自己的 `excerpt` 里。脱敏必须以“行”为边界：先收集整行全部命中区间、合并后一次性替换，再给每条 finding 生成同一份完整脱敏上下文。覆盖同一行双 generic 与 AWS + generic，store / HTTP / CLI / MCP / markdown 任一出口都不得含原值。另：MCP 的 `scope` / `format` 不得用 `z.string()` 在协议层抛 `-32602`，应放宽后在 handler 内统一报 `VALIDATION_FAILED`，与 HTTP / CLI 契约一致 |
