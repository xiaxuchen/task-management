...
| 39 | 上线 SQL 风险审查由既有 `release_items` 推导，不落表 | 上线 SQL 以 `kind=sql` 的 release item 登记（名称 + 内容 + 回滚 + 状态）。「内容有没有风险」是从这份内容推导出的**结论**，不是新的业务数据；落表会产生第二份真相，还要处理「内容改了结论要不要失效」。与 `acceptance_report` / `readiness` / `release_checklist` / 推送门禁同一条纯读原则：`buildReleaseSqlAudit` **不写库、不动 revision**，AI 可高频轮询而不制造变更噪声。规则集来自 `config.releaseSqlAudit.rules`：缺省用默认集，**非数组或空数组显式拒绝**（`VALIDATION_FAILED`）而非静默回退默认，免得「配置写错」与「想放宽规则」都变成悄悄用默认集、或让审查变成橡皮图章 |
| 40 | 危险 SQL 与风险提示分级：`danger` 阻塞、`warn` 只提示 | 把「不可逆破坏」与「风险写法 / 缺回滚」混为一谈会失去可执行性——前者必须拦住上线，后者只该提示补全。因此 `DROP TABLE/DATABASE` / `TRUNCATE` / 无 `WHERE` 的 `UPDATE` / `DELETE` 记 `danger` → `ready=false` 且进 `blockers`；`DROP COLUMN` / 缺回滚脚本记 `warn` → 单列 `warnings`，不改变 `ready`。**注释剥离 / `;` 分段 / `WHERE` 判定共享单次词法扫描**（`scanSql`）：三处各自做全文正则会让字符串字面量互相污染，独立测试已复现三类假结论——`'where'` 洗白无条件 `UPDATE`（D1）、`'--'` / 块注释符号吞掉后续 `DROP TABLE`（D2）、`';'` 错误切段把合法 `UPDATE` 判成缺 `WHERE`（D3）。扫描一次识别单/双引号与反引号、转义与成对引号、行注释与块注释，只在字符串 / 注释之外切 `;`，产出的代码区掩码供规则正则与 `WHERE` 判定使用。与决策 31/35/37 同一条纪律：边界判定只有一个真相来源。决策编号依赖 `../commit-push-gate/` 分支先合入（36/37/38），合并顺序决定最终编号 |
