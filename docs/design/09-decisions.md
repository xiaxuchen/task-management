...
| 39 | 验收签收落表并绑定证据指纹 | 测试通过 ≠ 业务验收。`acceptance_signoffs` 单独记录 `accepted/rejected`、签收人、意见与当时的 sha256 证据指纹；用例、期望或最近报告结论变化后签收自动 `stale`，交付门禁重新阻塞。避免“测试全绿即交付”绕过验收，也避免旧签收在测试变化后继续假绿 |
