...
- **单测（`node:test`）**：
  ...
  - `merge-integration`：`merges` 状态机 CRUD 与 state 校验；真 git 下 `precheckMerge` **只读**（冲突时 `merge-tree` exit=1 也能解析出冲突文件，不改目标/源分支）；`runMerge` 无冲突 `merge --no-ff` 落 `merged` + `merge_sha`、重复调用幂等；冲突落 `precheck_conflict` 且不改分支；缺 `confirm` / 分支缺失 / 非工作单元类型给稳定错误码
  ...
  - `business-gate`：业务检查门禁——空态 `ready=null`（不伪造成通过）；未关闭缺陷阻塞、`done` / `cancelled` 关闭；`biz_check` 用例 `not_run` / `running` / `fail` 阻塞、`pass` 放行；历史 `pass` 不掩盖后来的 `fail`；停用用例与其它 `kind` 不参与；`scope=self` 与 `subtree` 纳入差异；`blockers` 按 `open_defect` / `unpassed_case` 分型；非法 `scope` → `VALIDATION_FAILED`；**纯读聚合不产生 revision**；markdown 单元格转义；**报告 kind 与用例 kind 不一致的假绿回归**（写入侧 `VALIDATION_FAILED` 拒绝、省略 `kind` 时沿用用例 `kind`、无 `caseId` 才回落默认、读取侧忽略历史不一致行且验收报告同口径）
  - `business-gate-entrypoints`：CLI 真实子进程与 store 逐字段一致 / `--format md` / 非法 `--scope` 非零退出；MCP 真实协议与 store 一致、非法 `scope` 返回 `isError + VALIDATION_FAILED`（不泄漏 SDK `-32602`）
  ...
  - `workflow-map`：研发主线思维导图——阶段投影、四态 `pass/fail/pending/empty`、`scope=subtree` 多需求单元、非法 `scope` 拒绝、**纯读聚合不产生 revision**、上线分支稳定写引用、上线项与检查报告回写后状态翻转且只影响目标项、D1 裸项目/空白文档、D3 逐检查用例聚合、D4 `release_check` 上线项引用、markdown 渲染与能力清单登记
  - `format-validation`：D2 三入口 `format` 值域统一——`normalizeFormat` 拒绝非法值；HTTP 400 `VALIDATION_FAILED`、CLI 非零、MCP `isError`；合法 `md` 正常输出且只读不改 revision
