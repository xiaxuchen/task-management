...
- **单测（`node:test`）**：
  ...
  - `business-gate`：业务检查门禁——空态 `ready=null`（不伪造成通过）；未关闭缺陷阻塞、`done` / `cancelled` 关闭；`biz_check` 用例 `not_run` / `running` / `fail` 阻塞、`pass` 放行；历史 `pass` 不掩盖后来的 `fail`；停用用例与其它 `kind` 不参与；`scope=self` 与 `subtree` 纳入差异；`blockers` 按 `open_defect` / `unpassed_case` 分型；非法 `scope` → `VALIDATION_FAILED`；**纯读聚合不产生 revision**；markdown 单元格转义
