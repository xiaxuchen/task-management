...
- **单测（`node:test`）**：
  ...
  - `release-sql-audit`：上线 SQL 风险审查——四条 `danger` 规则各自独立命中（`DROP TABLE/DATABASE` / `TRUNCATE` / 无 `WHERE` 的 `UPDATE` / `DELETE`）；带 `WHERE` 不命中、**多语句按语句边界判 `WHERE`**（不被同段带 `WHERE` 的语句洗白）；大小写不敏感；注释里的关键字不误伤；`DROP COLUMN` / 缺回滚记 `warn` 不阻塞；非 `sql` 上线项不参与；`scope=subtree` 汇总；空态 `ready=null`；**纯读聚合不产生 revision**；markdown 单元格转义；三入口 1:1 与非法 `scope`/`format` 拒绝
