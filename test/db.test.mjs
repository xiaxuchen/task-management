...
test('openDb 建出全部表并开启 WAL / 外键', async () => {
  ...
  const tables = db
    ...
    .all()
    .map((r) => r.name)
  for (const t of ['nodes', 'attr_defs', 'attr_values', 'commits', 'mrs', 'documents', 'repos', 'merges', 'unit_repos', 'test_cases', 'test_reports', 'release_items', 'delivery_snapshots', 'meta']) {
