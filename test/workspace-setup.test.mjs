...
/** 真 git 仓库：main + `feature-send-receive` 分支（作为子需求基线） */
function makeRepo() {
...
test('ops setup：分支已存在但基线不同 → BRANCH_EXISTS_DIFFERENT_BASE（不静默复用）', async (t) => {
  ...
  t.after(() => fs.rmSync(repo.root, { recursive: true, force: true }))
  const { setupWorkspace } = await import('../server/ops.mjs')
  // 让基线真的前进一步，否则它与 main 同 tip、复用恰好是安全的（那是另一条用例）
  repo.g(['checkout', '-q', 'feature-send-receive'])
  fs.writeFileSync(path.join(repo.dir, 'base.txt'), 'base\n')
  repo.g(['add', '.'])
  repo.g(['commit', '-q', '-m', 'base work'])
  repo.g(['checkout', '-q', 'main'])
  // 造一个同名但基于 main 的分支（与已前进的基线 feature-send-receive 不同源）
