...
test('applyDesignOutline：写入「概要设计」文档，并让需求就绪门禁的 design_doc 通过', async (t) => {
  ...
  // requirement 节点由 xpx-120 起预置空白「概要设计」文档，因此写入是 upsert（created=false）而非新建
  assert.equal(out.results[0].created, false)
...
test('applyDesignOutline：dryRun 不写库、不动 revision，只回报将写哪些', async (t) => {
  ...
  assert.equal(store.getRevision(), before)
  // dryRun 不得写入内容：预置的空白「概要设计」仍保持空
  const preset = store.listDocuments(r.id).find((d) => d.name === '概要设计')
  assert.ok(preset, 'requirement 预置「概要设计」文档存在')
